...
export function PendingUploads() {
  ...
  async function load() {
    const { data } = await api.get("/admin/papers", { params: { status: "pending", search } });
  ...
  async function moderate(id, action) {
    const { data } = await api.patch(`/admin/papers/${id}/${action}`);
    if (action === "approve" && data.removedDuplicates > 0) {
      alert(data.message || `Removed ${data.removedDuplicates} duplicate pending upload(s).`);
    }
    load();
  }
  ...
  return (
    <section className="animate-fade-in">
      <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">Pending Uploads</h1>
      <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Review new uploads before they go live.</p>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                    <thead className="bg-slate-50 dark:bg-slate-800">
                      <tr>
                        {["Subject", "Shift/Month", "Type", ...(showUploaderEmail ? ["Uploader email"] : []), "Storage", "Actions"].map((head) => (
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {groupPapers.map((paper) => {
                        const isCloudinary = !paper.pdfPublicId?.startsWith("local/");
                        ...
                        return (
                          <tr key={paper._id} className="hover:bg-slate-50/30 dark:hover:bg-slate-850/20">
                            ...
                            <td className="px-4 py-3 text-slate-655 text-slate-500 dark:text-slate-400">
                            ...
                            <td className="px-4 py-3">
                              {isCloudinary ? (
                                <span className="inline-flex items-center rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-600/20 dark:bg-emerald-500/10 dark:text-emerald-400">
                                  Cloudinary
                                </span>
                              ) : (
                            ...
                            <td className="px-4 py-3">
                              <div className="flex flex-wrap gap-2">
                                <Button variant="secondary" onClick={() => setPreviewPaper(paper)} className="flex items-center gap-1"><Eye className="h-4 w-4" />Preview</Button>
                                <Button onClick={() => moderate(paper._id, "approve")} className="flex items-center gap-1"><Check className="h-4 w-4" />Approve</Button>
                                <Button variant="secondary" onClick={() => moderate(paper._id, "reject")} className="flex items-center gap-1"><X className="h-4 w-4" />Reject</Button>
